...
We evaluate the framework across two tiers: (i)~Tier-1 synthetic pair recovery ($n=200$), where own-country adapters achieve $70.0$--$78.5\%$ accuracy; and (ii)~Tier-2 empirical out-of-sample evaluation on $N=35$ unseen World Values Survey (WVS) Wave~7 items (30 mapped to the six \GPS{} dimensions; 5 demographic items). On non-\WEIRD{} targets (Mexico), the matched MEX adapter's fixed response distribution lies closer to Mexico's observed responses than the USA adapter's on $97.1\%$ of questions ($\Delta\text{TVD} = 0.1495$, $95\%$ CI: $[0.0964, 0.2152]$); with unconditioned prompts this difference can only arise from the adapter weights, and it establishes distributional alignment rather than country-conditioned inference. Conversely, on US targets the USA adapter degrades fidelity to the WVS response distributions relative to the base model; the mechanism (e.g., a pretraining-prior interaction versus overfitting to the synthetic scenario format) is not identified by this design. Option log-likelihood scoring exhibits systematic under-dispersion relative to human populations. We formalize the measurement argument through a Cronbach--Meehl nomological network linking the \GPS{} source instrument, the DPO training domain, and the WVS criterion domain, and we delineate reliable applications (relative comparative statics, pre-field instrument stress-testing) from unreliable ones (absolute headcount polling, fine-tuning over already-dominant priors).
...
The network is a \emph{joint} claim, and its links can fail independently---which is precisely what the data show. The USA adapter \emph{passes} Link~1 (Tier-1 accuracy $0.70$) but \emph{fails} Links~2--3 on US WVS items: the base model already encodes a strong US pretraining prior, so the adapter's additional synthetic signal over-corrects rather than refines. This is not evidence against the construct; it is evidence that the \emph{source domain} is redundant with the model's pretraining for WEIRD populations. The same network then correctly predicts that the MEX adapter---trained on a population outside the pretraining prior---should pass all three links, which it does on the aggregate Tier-2 metrics (Tier-1 accuracy $0.785$; matched-better fraction $97.1\%$ on MEX items; dimension-level convergence such as Altruism TVD falling from $0.450$ to $0.152$). Two caveats are warranted. First, matched-vs-cross compares each adapter's \emph{fixed} output distribution against each country's observed responses---the underlying option probabilities are country-invariant by design, so the claim is about distributional alignment, not inference-time conditioning. Second, on Mexico items the improvement relative to the \emph{base} model is concentrated in altruism, patience, and positive reciprocity; trust and risk-taking items improve relative to the cross adapter but not relative to the base model.
...
\emph{Plain-language reading.} This is the most important negative finding in the paper, and its interpretation is under active investigation. ``Pretraining prior'' refers to the implicit population the model assumes when asked an English-language survey question with no country context---a population that looks American because the bulk of its training corpus is American English. On US items, adding synthetic US signal on top of that prior moves the policy away from the empirical US WVS distribution (TVD $0.396 \rightarrow 0.526$); the USA adapter also moves away from Mexico's distribution (TVD $0.521 \rightarrow 0.638$), so the degradation is not specific to US items. The asymmetry with Mexico---where the matched adapter improves fit---is consistent with the prior-collision account (a weak base prior for Mexico versus a saturated one for the US), but competing explanations (adapter overfitting to the synthetic pair format, insufficient US-pair diversity) remain viable. The pre-registered persona-gradient protocol in the construct-validity layer is designed to adjudicate between these candidates; until it runs, the mechanism should be treated as unresolved. The practical takeaway that \emph{structural preference estimation works where the base model does not already have an opinion} is a hypothesis, not an established conclusion.
