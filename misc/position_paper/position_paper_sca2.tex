...
Anchor relevance & real-anchor versus permuted-anchor placebo adapters & PENDING --- restricted permutation placebo not yet run; required before claiming anchor information beyond generic synthetic tuning & apparent gains may reflect generic synthetic tuning rather than anchor information\\
...
The results dissociate the two routes into the model. The country-named prompt buys distributional shape (TVD $0.375$, best dispersion and top-match) but carries no \GPS{} ordering on trust ($\rho=0.06$) and overshoots the development gradient ($+0.73$). The trained weights carry the \GPS{} direction --- trust bridge $\rho=0.70$, development partials $+0.56$/$+0.46$, held-out pair accuracy up to $0.99$ --- but pay for it in concentration and in TVD. The anti-leakage price is $\approx 0.09$ TVD: the adapters' pooled TVD ($0.469$) exceeds the persona's ($0.375$) by $0.094$. What we cannot claim follows: histogram reproduction, country-specific dispersion, or magnitudes. These adapters are identified for preference \emph{direction} with transparent provenance, not for survey-distribution point prediction.
