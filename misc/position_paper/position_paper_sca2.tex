...
The phrase ``falsifiable'' is useful only if it names propositions, estimands, and failure consequences \cite{dell2026,cvprofiles2026}. This working-paper version supplies the test architecture, but not a public timestamped registration or final numerical thresholds; it should therefore be read as a proposed rejection design. The archived analysis plan fixed the unit of analysis, primary metric, comparison, practical margin, randomization/materialization count, uncertainty procedure, and country/domain/protocol decision rule for every row in Table~\ref{tab:tests} before outcomes were inspected. Each test concerns a distinct link; passing one does not imply passing another.
