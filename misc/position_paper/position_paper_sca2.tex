...
\subsection{Cultural distance: specificity at chance, and the PCA geometry}
On the CF\_ST cultural-distance geometry of Muthukrishna et al.\ (2020), the median rank of an adapter's own country among the 42 is $20.5$ (chance $21.5$); 11 of 16 adapters are nearest to Canada, a mid-scale reference; the best adapters (GBR rank 2, RUS rank 4, BRA rank 8) still sit far from their own countries (Figure~\ref{fig:cdmaps}, left). The two-axis PCA of the mapped items tells the same story: adapter and persona positions track the model family, not the target country (Figure~\ref{fig:cdmaps}, right). No distributional specificity is recovered.

The coarse structure that does survive is the sign profile: the 76-country \GPS{} bank collapses to 35 sign($z$) profiles (22 multi-country classes), and the sixteen adapters instantiate 13 distinct profiles (MEX$\equiv$RUS, IND$\equiv$GRC, IDN$\equiv$EGY). But sign-twins are a development shadow: twin proximity on human \WVS{} is explained by shared GDP (partial correlation with the GDP gap $-0.09$). Country-level sign patterns are not cultural families.
