...
\textbf{Status of this version.} Methods-first working paper. The protocol, rejection design, and claim boundary remain the spine. A 16-country Llama-3.1-8B adapter bank now exists (H1 GPS-pair recovery $0.89$--$0.99$; H2 WVS histogram fit loses to a country-named prompt and to uniform noise). Those results are treated as a proof-of-concept diagnostic of what the current operator is identified for, not as filled Table~\ref{tab:resultshell} cells and not as a Nature/PNAS research-article claim. The USA/Mexico two-country pilot remains Appendix~A only. This draft is not a public preregistration.\vspace{0.25em}}}
...
This paper makes four contributions: a formal statement of the aggregate-anchor construction problem and its scientific object; a modular protocol with anti-leakage, provenance, and reproducibility requirements; an empirically testable design built on five links---anchor transmission, policy encoding, anchor relevance, external transport, and operator robustness; and a predeclared multi-country comparison of the constructed policy against base-model, country-prompt, non-LLM, and permuted-anchor baselines. A 16-country implementation now exists as a proof-of-concept diagnostic of the current operator (Discussion); it does not fill Table~\ref{tab:resultshell}, which remains reserved for a frozen, predeclared panel.
...
A 16-country Llama-3.1-8B implementation of the current protocol (synthetic GPS-sign pairs, $G=\mathrm{sign}(z)$, unconditioned evaluation prompts) is already informative as an operator diagnostic, and only as that. Held-out GPS-pair recovery is high ($0.89$--$0.99$). On independent WVS Wave-7 histograms the country-named prompt and even a uniform noise floor beat the adapters on total variation distance, while the adapters---not the prompt---rank countries with GPS on the confirmatory trust composite. Pairwise DPO is therefore identified for preference \emph{direction} under a declared aggregate anchor, and is not identified for population marginals. That bound is the empirical content of the construction, not a reason to abandon it. Filling Table~\ref{tab:resultshell} still requires a frozen country frame, a restricted permutation placebo, a named non-LLM baseline, and a predeclared decision rule; those cells remain empty here.

The same diagnostic also disciplines venue. Nature, Science, and a PNAS research article would correctly desk-reject a 16-country adapter study whose primary external metric loses to a one-line country prompt. A more prestigious outlet than \emph{Review of Experimental Economics} is still feasible if the paper stays methods-first: \emph{PNAS Nexus} is the realistic target now (Tao et al.\ already published cultural-alignment evaluation there); \emph{Nature Human Behaviour} or \emph{Science Advances} remain stretch goals after a 4-country extended-prompt subset, a permutation placebo, and a non-LLM baseline are in hand; \emph{Sociological Methods \& Research} and \emph{Political Analysis} are the strongest field homes for the negative transport result. Experimental economics is the SCA1 fallback, not the first try.
