...
\vspace{0.35em}
\noindent{\small\textbf{Corresponding author:} Augusto Gonzalez-Bonorino, EconLLM Lab and Arizona State University. Email: \texttt{agonz439@asu.edu}.}
...
What the construction does not do is equally sharp. It does not reproduce histograms. It does not recover magnitudes or within-country heterogeneity. It does not license treating sixteen adapters as sixteen cultures. Training-side operator ablations remain unrun; until they exist, the direction result is a property of this materialized bank, not of every protocol in the family. The useful applications are those that need a transparent, rejectable preference \emph{direction}---item design, benchmark construction, exploratory measurement. The method is not a substitute for human data and should not be used to speak for individuals, subgroups, or policy counterfactuals.


\section*{Code, data, and disclosures}
Analysis code and the committed table artifacts live at \url{https://github.com/EconLLM-Lab/SCA2_PofW} (commit \texttt{a336ac0} and descendants). Headline numbers in Section~7 are regenerated by \texttt{analysis/phase2/13\_unified\_comparison.py}, \texttt{17\_anchor\_permutation\_placebo.py}, and \texttt{18\_2x2\_analysis.py} from banked option-probability files. Those raw evaluation banks are not in the public tree; they will be deposited with a DOI before journal submission. Adapter weights are on the Hugging Face hub (\texttt{Bonorinoa/SCA2-phase2-adapters}). \GPS{} data are documented by Falk et al.\ \cite{falk2018}; \WVS{} Wave~7 data are available from the World Values Survey Association under DOI \url{https://doi.org/10.14281/18241.18} \cite{wvs2022}. Raw survey microdata are not redistributed.
