...
\vspace{0.6em}
% Figure 1: systems view of knowledge production (from Augusto's sketch).
% Figure 2: core objects / epistemic ownership (per the 2026-08-10 figure audit).
...
\begin{tikzpicture}[
  ...
  lane/.style={draw=econblue, rounded corners=3pt, fill=mist, align=center,
               text width=1.02in, minimum height=0.78in, inner sep=4pt, line width=0.7pt},
  tool/.style={draw=slate!70, dashed, rounded corners=2pt, fill=white, align=center,
               text width=0.98in, minimum height=0.55in, inner sep=3pt, font=\scriptsize},
  cvbox/.style={draw=passgreen, rounded corners=3pt, fill=passgreen!10, align=center,
                text width=1.55in, inner sep=4pt, line width=0.8pt},
...
\node[lane, right=0.18in of h] (g) {\textbf{Measurement}\\\textbf{generation}\\\scriptsize\color{slate}how to operationalize};
\node[lane, right=0.18in of g] (v) {\textbf{Construct \&}\\\textbf{measure validation}\\\scriptsize\color{slate}which interpretations};
\node[lane, right=0.18in of v] (e) {\textbf{Estimation \&}\\\textbf{inference}\\\scriptsize\color{slate}what follows};
\node[lane, right=0.18in of e] (c) {\textbf{Conclusion}\\\scriptsize\color{slate}what survives choice};
\draw[arr] (h)--(g); \draw[arr] (g)--(v); \draw[arr] (v)--(e); \draw[arr] (e)--(c);
\node[tool, below=0.5in of h] (ht) {theory,\\ prior evidence,\\ registry};
\node[tool, below=0.5in of g] (gt) {surveys, experiments,\\ LLMs, embeddings,\\ trained policies};
\node[tool, below=0.5in of e] (et) {regression, causal ID,\\ prediction, ranking};
\node[tool, below=0.5in of c] (ct) {robust result set\\ $\Bset^*$, $[L,U]$};
\draw[arr] (h)--(ht); \draw[arr] (g)--(gt); \draw[arr] (e)--(et); \draw[arr] (c)--(ct);
\node[cvbox, below=0.5in of v] (cv) {\textbf{\texttt{cvprofiles}} (validation lane)\\[1pt]
...
\draw[decorate,decoration={brace,amplitude=3pt}, color=slate]
  ([yshift=0.06in]h.north west) -- ([yshift=0.06in]g.north east)
...
\draw[decorate,decoration={brace,amplitude=3pt}, color=econblue]
  ([yshift=0.06in]v.north west) -- ([yshift=0.06in]c.north east)
  node[midway, above=4pt, font=\scriptsize\bfseries\color{econblue}] {Empirical science};
\node[tag, align=center, anchor=north] at (cv.south) [yshift=-0.30in]
...
\draw[darr] (et.west) to[out=180,in=0] (cv.east);
\end{tikzpicture}
...
Solid arrows = artifacts, dashed = researcher-owned commitments, and the diagnostics that monitor the validation lane are reported separately from the headline outputs.}
\label{fig:systems}
\end{figure}
...
\begin{tikzpicture}[
  ...
  upbox/.style={draw=slate, dashed, rounded corners=2pt, fill=white, align=center,
                text width=1.05in, minimum height=0.7in, inner sep=3pt},
  ...
  downbox/.style={draw=econblue, rounded corners=2pt, fill=mist, align=center,
                  text width=1.15in, minimum height=0.7in, inner sep=3pt},
  diagbox/.style={draw=amber, dotted, rounded corners=2pt, fill=amber!8, align=center,
                  text width=1.15in, minimum height=0.55in, inner sep=3pt},
  ...
  pinbox/.style={draw=warm, rounded corners=2pt, fill=warm!8, align=center,
                 text width=0.85in, minimum height=0.5in, inner sep=3pt}]
\node[font=\scriptsize\bfseries\color{slate}] at (0,1.5) {Upstream};
\node[upbox] (gen) at (0,0.35) {\textbf{Generators}\\\scriptsize survey, LLM,\\ multimodal, policies};
\node[upbox] (C2) at (0,-0.8) {\textbf{Construct $C$}\\\scriptsize prose definition};
\node[box] (menu) at (2.6,0.35) {\textbf{Menu $\M$}\\\scriptsize unit$\times$score table};
\node[font=\scriptsize\bfseries\color{econblue}] at (5.3,1.5) {Validity layer};
\node[box] (net) at (5.3,0.35) {\textbf{Network}\\\scriptsize $\Rset$, $\tau$, $\delta$; aux $V$};
\node[box] (hold) at (5.3,-0.8) {\textbf{Holdout}\\\scriptsize stage and/or units};
\node[font=\scriptsize\bfseries\color{passgreen}] at (8.0,1.5) {Object of inference};
\node[msbox] (ms) at (8.0,0.35) {\textbf{Admissible set}\\\scriptsize $\M^*$};
\node[font=\scriptsize\bfseries\color{econblue}] at (10.9,1.5) {Scientific outputs};
\node[downbox] (b) at (10.9,0.35) {\textbf{Downstream}\\\scriptsize $\beta$ on survivors};
\node[downbox] (range) at (13.0,0.35) {\textbf{Robust result}\\\scriptsize $\Bset^*$, $[L,U]$};
\node[diagbox] (band) at (10.9,-0.8) {\textbf{Uncertainty band}\\ empty rate, boundary};
\node[diagbox] (grid) at (13.0,-0.8) {\textbf{Sensitivity}\\ $\tau$-grids, diagnostics};
\draw[arr] (gen)--(menu); \draw[arr] (C2)--(net);
\draw[arr] (menu)--(net); \draw[arr] (net)--(hold);
\draw[arr] (hold)--(ms); \draw[arr] (ms)--(b); \draw[arr] (b)--(range);
\draw[dottedarr] (ms.south) -- (band.north);
\draw[dottedarr] (band)--(grid);
\node[pinbox] (pin) at (6.7,-1.9) {\textbf{Freeze / run-id}\\\scriptsize pins commitments};
\draw[dottedarr] (pin.north) -- (hold.south);
\node[font=\scriptsize\color{slate}, text width=5.7in, anchor=west] at (-0.6,-2.35)
