...
This paper makes four contributions: a formal statement of the aggregate-anchor construction problem and its scientific object; a modular protocol with anti-leakage, provenance, and reproducibility requirements; an empirically testable design built on five testable links; a sixteen-country comparison against base-model, country-prompt, non-LLM, and permuted-anchor baselines; and a stated use as a reusable scoring instrument on new batteries that theoretically load on the same six preference dimensions.
...
The phrase ``falsifiable'' is useful only if it names propositions, estimands, and failure consequences \cite{dell2026,cvprofiles2026}. This paper is not a public preregistration. The archived analysis plan fixed the unit of analysis, primary metric, comparison, practical margin, randomization count, uncertainty procedure, and decision rule for every row in Table~\ref{tab:tests} before outcomes were inspected. The five links have been exercised by the sixteen-country evaluation; per-link status is in Table~\ref{tab:tests}. Each test concerns a distinct link; passing one does not imply passing another.
...
Operator robustness & scenario, generator, selector, scorer/QC, materialized-data, and training-seed ablations & PARTIAL --- inference-time temperature scaling measured (pooled TVD $0.469\to0.332$ at $T=3$, top-option match unchanged); weights-by-prompt interaction measured (bridge $\rho$: adapter-persona $0.60$, adapter-uncond $0.78$, base-persona $0.06$; shape unchanged) --- direction survives prompting; training-side ablations pending (extended-prompt bank, soft-\DPO{}/$|z|$ pruning, repeated seeds, deterministic-vs-legacy selector) & the conclusion depends on arbitrary or unstable upstream choices\\
\bottomrule
\end{tabularx}
\end{table}
...
External transport is a predictive-criterion claim: the policy's induced option probabilities improve alignment with independently collected human response distributions. It is neither recovery nor validation of a complete country distribution. For a question $q$ with valid options $\mathcal{Y}_q$, let $\ell_{\widehat\pi_c}(y\mid q)$ denote the total log probability assigned to the token sequence encoding answer option $y$ under a fixed prompt and option-order rule. The policy yields the option probabilities in equation~\eqref{eq:choiceprob},
\begin{equation}
\widehat P_c(y\mid q)=\frac{\exp\{\ell_{\widehat\pi_c}(y\mid q)\}}{\sum_{j\in\mathcal{Y}_q}\exp\{\ell_{\widehat\pi_c}(j\mid q)\}},
\label{eq:choiceprob}
\end{equation}
and the primary descriptive discrepancy is the total variation distance in equation~\eqref{eq:tvd} (with frequency-based fixation indices \cite{muthukrishna2020} as a dispersion-sensitive alternative),
\begin{equation}
...
The implementation uses \GPS{} as the anchor source and \WVS{} Wave 7 as the primary independent outcome surface. The country frame is the sixteen-country panel reported in Section~7, drawn from the \GPS{}$\times$\WVS{} intersection before outcomes were inspected. The evaluation battery is the theory-led map of thirty single-choice items plus three demographic items; confirmatory claims are restricted to trust and, directionally, patience. The remaining dimensions are reported as imprint diagnostics, not as recovered survey constructs.
...
Alternatives exist if the criteria recommend them: \WVS{}/EVS country means, with the caveat of overlap with typical evaluation surfaces; Schwartz value country scores (PVQ/ESS) \cite{schwartz1992}; Hofstede dimensions \cite{hofstede2010}; Inglehart--Welzel cultural-map coordinates \cite{inglehart2005}; and the GLOBE project \cite{globe2004}. Table~\ref{tab:anchor-sources} lists these candidates with their principal caveats. The method is agnostic to the specific source, so long as the criteria above hold.
