...
% -----------------------------------------------------------------------------
% Measuring What We Mean — position paper v8 (2026-08-14)
% Construct-identified inference (composition of CV, multiverse, PI).
% Engine: cvprofiles 3.0.0 (released on PyPI 2026-08-10); WVS/GPS patience
% application as initial real-data proof of work (frozen scores/network/seed).
% -----------------------------------------------------------------------------
...
This version reports the \WVS/\GPS{} patience application under a freeze of the score matrix, network, split, seed, and package pin (\cvp{}~3.0.0; verifier \texttt{tools/verify\_wvs\_gps.py}): a \GPS{} positive control, \WVS{} proxies and a composite, open-weight \LLM{} log-probability scores, and a noise negative control, facing one common validity network with held-out country evidence and a downstream estimand.
The example demonstrates the scientific contract rather than settling the measurement of patience.
...
We compose three traditions—construct validity, multiverse reporting, and partial identification—around four objects: a construct $C$, a finite menu $\M$, a theory-derived validity argument $\Rset(C)$ written as restrictions, and a downstream estimand $\beta(m)$.
Surviving measures form $\M^*$; the image $\Bset^*$ reports which conclusions survive remaining measurement choice.
A country-level patience application places a $\GPS{}$ positive control, $\WVS{}$ proxies and a composite, two open-weight log-probability scores, and a noise negative control under one network, with held-out countries and a random-selection floor.
The selection rule can fail on held-out evidence.
This paper states the composition and reports an initial proof of work; the experiments that would decide whether the composition has empirical bite are in Section~6.5.
...
Selection is made prospectively falsifiable by holding out restrictions or units, and sampling fragility induced by data-dependent screening is reported additively, as an uncertainty band rather than a confidence set for the headline range.
...
The headline admissible set retains the \GPS{} measure and the 8B prompt measure, with range $[0.328,0.402]$ (OLS coefficient of the standardized measure on log GDP per capita, education control, $n=41$), at the 100th percentile of the random-selection null; the pooled robust set is empty.
The example demonstrates the scientific contract rather than settling the measurement of patience.
...
The patience application is a proof of work for that contract, not a general standard journals should require.
...
The resampling unit must be declared so that every slack and every $\beta(m)$ is a function of the resampled units; in the patience application the unit is the country and the \LLM{} columns are frozen maps, not resampled pretraining draws.
...
The example demonstrates the scientific contract---heterogeneous measures, one common validity network, held-out country evidence, negative controls, and a downstream estimand---rather than settling the measurement of patience.
...
Engineering origin confers no standing: the \GPS{} instrument, the survey proxies, the composites, and the model scores all face the same construct-level network.
...
$m_{\mathrm{wvs\_q13}}$ & WVS~7 Q13 (thrift), country mean & Cheap survey proxy; semantically related, not a positive control. \\
$m_{\mathrm{wvs\_q14}}$ & WVS~7 Q14 (perseverance), country mean & Cheap survey proxy; semantically related, not a positive control. \\
...
Reporting posture is stated explicitly: $\M^*_{\mathrm{select}}$ is the headline object by a 2026-08-10 amendment after a single 80/20 split admitted noise on every held-out bar and returned an empty robust set.
Per-fold compliance remains a diagnostic.
The redesign is a change of \emph{test}, recorded with the earlier runs; it is not prospective falsifiability of the original contract.
Assumption~A.5 applies to the frozen amended contract, not to the discarded split.
...
\textbf{(iii) The robust set is empty.} No measure complies on all five held-out frames ($m_{\mathrm{prompt\_a}}$ 3/5, $m_{\mathrm{gps\_patience}}$ 1/5; noise and Q13 once each), so $\M^*_{\mathrm{robust}}=\varnothing$.
Under posture~(a) emptiness does not replace the select-set range.
It is reported as a finding: the hold-frame screen is low-powered at $n_{\mathrm{hold}}\approx 8$, and the validated instrument fails the monotonicity restriction we wrote (fold-0 slack $-0.383$ against $\tau=0.15$).
A reader may treat that failure as evidence against this $\Rset(C)$ at country level.
\textbf{(iv) The discriminant threshold} was re-anchored from $0.30$ to $0.35$ before the accepted freeze, on published country-level correlations in $[0.23,0.36]$; the $0.30$ run is retained as a motivating diagnostic.
...
This is an observation about country-level aggregation, not an individual-level claim, and it is exactly the kind of signal a menu-level screen exposes that a single retained operationalization would hide.
...
The fold-0 convergent slack of $m_{\mathrm{prompt\_b}}$ is $-0.059$; a correlation standard error of order $1/\sqrt{41}$ puts several slacks on the boundary, so this design does not meet the separation condition of Proposition~A.1.
Section~\ref{sec:uncertainty} requires an uncertainty band, empty-replicate rate, and boundary flags as an additive layer on this frozen score matrix; those objects are implemented in \cvp{}~3.0.0 and are not in the accepted summary.
...
That unrestricted span is exactly \eqref{eq:range} with $\Rset=\emptyset$.
...
AI agreement is not independent validation: the two prompt arms share pretraining families, so co-admission of $m_{\mathrm{gps\_patience}}$ and $m_{\mathrm{prompt\_a}}$ is not two independent validations.
The Independence and exclusions fields of Table~\ref{tab:adapter} are required records for those arms and are not filled here.
...
\label{sec:agenda}
This version reports an initial demonstration.
The following experiments, not this application, decide whether the composition changes scientific conclusions.
\begin{enumerate}[label=(\roman*)]
