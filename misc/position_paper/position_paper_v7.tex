...
% -----------------------------------------------------------------------------
% Measuring What We Mean — position paper v7 (rev. 2026-08-09)
% Fusion: v5 general-science spine + v4 formal payload + first-principles SI
% Engine: cvprofiles 2.5.2 (current tagged baseline; holdout + coverage)
...
\pagestyle{fancy}
\fancyhf{}
\fancyhead[L]{\small\color{slate} EconLLM Lab \textperiodcentered\ Position paper v7}
...
The companion package \cvp{}~2.5.2 now ships the holdout workflows (restriction-stage tags and units-split) and the additive coverage uncertainty band under synthetic gates; those layers are illustrated in Section~\ref{sec:dgp}.
...
Code: \href{https://github.com/EconLLM-Lab/SCA2_PofW}{\texttt{SCA2\_PofW}}; engine: \href{https://github.com/Bonorinoa/cvprofiles}{\texttt{cvprofiles}} (tag \texttt{v2.5.2}).
\vspace{0.3em}}}

\vspace{0.85em}
\section*{Statement of significance}
Scientists rarely observe constructs such as trust, ideology, population preference profiles, or risk preference directly. They work through measures.
...
Measurement selection is formulated around four objects: a construct $C$, a declared menu of candidate measures $\M$, a set of theory-derived validity tests $\Rset$, and a downstream target $\beta(m)$ computed using measure $m$.
The tests define a surviving set $\M^*$; the image $\Bset^*=\{\beta(m):m\in\M^*\}$ reports the conclusions robust to measurement choice within the declared menu and validity argument.
...
It is implemented in the open, model-free package \cvp{}~(2.5.2).
...
measurement selection; construct validity; partial identification; AI measurement; synthetic cultural agents; preference profiles; trust
...
Trust, patience, reciprocity, ideology, polarization, and population preference profiles are familiar examples.
...
In Hall's hand-coded analysis of the PolMeth XLIII program, 45 of 175 presentations (26\%) were classified as core generative-AI; annotation plus measurement accounted for 27 of those 45 presentations (60\%). These are program-title and tag classifications, not a content analysis of full papers~\cite{hall2026}.
Baumann et al.\ replicate 37 annotation tasks from 21 published social-science studies and show that a handful of prompt paraphrases can make virtually anything statistically significant. Across 2,361 realistic hypotheses, they find incorrect conclusions for approximately 31\% of hypotheses for state-of-the-art LLMs~\cite{bauman2026}.
...
In the 2026 NBER Methods Lectures, Dell and Rambachan frame construct validity as partial identification over a researcher-specified menu and moment restrictions. Dell's construct-validity lecture defines the admissible set and downstream range, while the broader lecture program separately treats validation samples, repeated measurements, and data fusion~\cite{dellrambachan2026,cronbach1955}.
Their account notes that additional restrictions can narrow the set of plausible measures and calls for principled, widely accepted methods for construct validity.
...
O\u{g}ut and Yin bound a regression coefficient using multiple nonlinear measurements, reaching the same $[L,U]$ destination without an admissible set or a selection rule~\cite{ogutyin2026}.
...
A standard analysis begins from one operationalization---a survey item, a multi-item composite, a text score, or a model-generated proxy---and reports a point estimate of $\beta(m)=\mathrm{Corr}(m,Y)$ or a related regression coefficient.
When candidate operationalizations are cheap, the same design admits a \emph{menu} $\M$ of such scores.
Validity is not read off $\beta(m)$.
...
Measures that satisfy the network form the surviving set $\M^*$; the image $\Bset^*=\{\beta(m):m\in\M^*\}$ is the set of conclusions licensed under that validity argument.
...
\node[box, below=0.85cm of Ms, fill=passgreen!12, draw=passgreen] (Th)
  {\textbf{Robust set $\Bset^*$}\\[2pt]{\scriptsize\color{slate}Which conclusions survive?}};
\draw[arr] (C)--(M); \draw[arr] (M)--(R); \draw[arr] (R)--(Ms);
\draw[arr] (Ms)--(Th);
\node[font=\scriptsize\color{slate}, left=0.15cm of Th, align=right, text width=4.2cm]
  {Propagate measurement choice\\into the scientific result $\beta(m)$};
\end{tikzpicture}
...
\node[outbox] (ms) at (10.2,0.35) {\textbf{$\M^*$, $\Bset^*$}\\survivors-only range};
...
Finally, $\beta(m)$ denotes the scientific result obtained when measure $m$ is used in the downstream analysis.

For an empirical application, $i=1,\ldots,n$ indexes units in a declared universe and $D=\{(X_i,V_i,Y_i)\}_{i=1}^n$ denotes the frozen analysis data. The population objects are $\M^*$, $\Bset^*$, and $[L,U]$; their reported plug-in analogues are $\widehat{\M}^*$, $\widehat{\Bset}^*$, and $[\widehat L,\widehat U]$. This distinction matters: the engine reports a finite-menu, assumption-indexed measurement-robust object, not a nonparametric identification result for the latent construct itself.
...
Validation as defined here is distinct from two adjacent activities~\cite{dellrambachan2026}: \emph{sensitivity analysis} asks whether conclusions change across procedures---the role of threshold grids over $\tau_r$, of $\theta$- and $\delta$-grids, and of the uncertainty band in Section~\ref{sec:theta}---whereas \emph{hypothesis testing} characterizes sampling variability of a target $\beta(m)$ given a fixed measure.
...
\label{eq:Mrobust}
\end{equation}
and the headline range is the image of $\beta$ on $\M^*_{\mathrm{robust}}$ (with $\beta$ evaluated on the full pooled sample over that set).
...
In the open implementation (\cvp{}~2.5.2), the freeze/run-id contract hash-pins the network, anchors, stage split, holdout unit list, and seed before holdout scoring~\cite{cvprofiles}---the operational meaning of ``preregistered'' here.
...
Let $\beta(m)$ denote that result when measure $m$ is used.
Once the surviving set is defined, report
\begin{equation}
\Bset^*(C;\M,\Rset)=\bigl\{\beta(m):m\in\M^*\bigr\}.
\label{eq:Theta}
\end{equation}
For a scalar target, a transparent summary is the assumption-indexed admissible-measure range
\begin{equation}
\bigl[L,\,U\bigr]
=\Bigl[\min_{m\in\M^*}\beta(m),\;\max_{m\in\M^*}\beta(m)\Bigr].
\label{eq:range}
\end{equation}
We call $\Bset^*$ the \emph{robust result set}.
...
A narrow $\Bset^*$ supports a result robust to surviving measurement choice; a wide or sign-changing $\Bset^*$ is a substantive finding about the fragility of the scientific conclusion.

\subsection{Inference over the admissible set}
The headline object remains the plug-in range \eqref{eq:range}: the image of $\beta$ on $\M^*$, with no model and no shrinkage.
...
\item \emph{Construct validation / menu-level partial identification}---restrictions $\Rset$, survivors $\M^*$, and robust conclusions $\Bset^*$;
...
Synthetic Cultural Agents are therefore one family of measurement-generation technologies for culturally patterned preference profiles~\cite{capra2025}.
The demanding application is a preference profile, not culture simpliciter: six GPS dimensions are a bounded vector of economic preferences that may be culturally patterned, not an ontological measure of culture as a whole. Measurement selection is the general methodological contribution.
...
The examples use restriction types and target functionals available in \cvp{}~2.5.2 (\texttt{corr\_min}, \texttt{corr\_zero}, \texttt{mean\_order}, and \texttt{corr\_y}, among others)~\cite{cvprofiles}.
...
Auxiliaries are a convergent criterion $V_{\!c}$, a discriminant foil $V_{\!d}$, a binary known-groups indicator $G$, and an outcome $Y$ used by the target functional $\beta(m)=\mathrm{Corr}(m,Y)$.
The menu is
\begin{equation}
...
The admissible-result set is
\begin{equation}
\Bset^*=\bigl\{\mathrm{Corr}(m_{\mathrm{true}},Y),\,\mathrm{Corr}(m_{\mathrm{weak}},Y)\bigr\}
=\bigl\{0.54,\,0.30\bigr\},
\qquad [L,U]=[0.30,\,0.54].
\label{eq:dgpa-range}
\end{equation}
...
Population slacks for $\Rset_S$ and target $\beta(m)=\mathrm{Corr}(m,Y)$.
...
\toprule
Measure & $s_{\mathrm{conv}}$ & $s_{\mathrm{disc}}$ & $s_{\mathrm{grp}}$ & $\beta(m)$ & In $\M^*$? \\
\midrule
...
\cvp{}~2.5.2 recomputes $\M^*$ inside a units bootstrap, reports an empty-replicate rate, labels an additive uncertainty band, attributes boundary measures via $|\mathrm{margin}_m|\le\kappa\cdot\mathrm{SE}_m$, and its Python API exposes both restriction-stage and units-split holdout.
...
\cvp{}~2.5.2's Python API already exposes stage-tagged restrictions, units-split composition, and holdout verdicts under synthetic gates.

\subsection{Finite-sample engine check (not headline numbers)}
To verify that the shipped pipeline reproduces the qualitative structure of DGP~A--D, we drew $n{=}2000$ standardized units from the linear factor design of Table~\ref{tab:loadings} (seed $20260808$) and ran \cvp{}~2.5.2 with bootstrap ($n_{\mathrm{boot}}{=}400$, $\alpha{=}0.10$, $\kappa{=}2$).
...
They do \emph{not} show that real survey items, persona prompts, or preference-trained policies survive any particular network for trust or a population preference profile.
...
The coupled-inference reporting layer, holdout-restriction workflow, and evaluator coverage required by Sections~\ref{sec:holdout}--\ref{sec:theta} now ship in \cvp{}~2.5.2 under synthetic gates~\cite{cvprofiles}.
...
The flagship illustration takes the preference-profile application end to end---from observation of population moments to the falsification of a measure-selection rule. A benchmark-design application is developed separately in Appendix~\ref{app:bench}; it is a forward-looking domain extension, not a second empirical contribution of this paper.

\subsection{Flagship illustration: preference profiles, SCA~2 versus persona prompting}
The flagship application compares two AI measurement technologies---persona prompting and the SCA~2 adapter---against conventional instruments for a culturally patterned preference profile, under one common validity argument, from observation to falsification.
...
\textbf{Construct $C$:} a population-level preference profile across six GPS dimensions; generalized interpersonal trust---the disposition to expect cooperative behavior from anonymous others in the absence of contractual enforcement---serves as the first focal construct, defined independently of any single survey instrument~\cite{glaeser2000,sapienza2013,butler2016}.
...
\centering
\small
\caption{Preference-profile menu for the flagship illustration.
...
Cultural / persona prompt & Pretrained model + declared population context & Direct identity at inference: the population is named in the prompt; standard inference-time synthetic-subject baseline. \\
...
Primary $\beta(m)$: association of the trust measure with a preregistered economic or behavioral criterion (and, secondarily, cross-country ranking fidelity where the criterion is defined).
Report $\M^*$, $\Bset^*$, the uncertainty band, empty-replicate rate, and boundary set.
...
Measurement choice changes conclusions & $\Bset^*$ for consequential analyses & Specification curves; multiverse & Surviving measures always yield indistinguishable results \\
...
\textbf{v2.5.2 (current tagged package baseline).}
 The implementation provides the score $\to$ restrict $\to$ identify $\to$ report state machine, a survivors-only range, and empty $\M^*$ as an exit-success finding. It pins frozen inputs through a run-id and supports additive bootstrap, $\theta$- and $\delta$-grid diagnostics, restriction-stage and units-split holdouts, and an uncertainty band with empty-replicate and boundary reporting. Its registry includes correlation, ordering, and rank restrictions and the \texttt{corr\_y}, \texttt{ols\_coef}, \texttt{diff\_means}, and \texttt{map\_distance} target functionals. The engine is model-free.
...
% =============================================================================
\section{Discussion: a human-owned validation gate}
\label{sec:discussion}
% =============================================================================
The four-layer separation of Section~\ref{sec:ai}---hypothesis generation, measurement generation, construct validation, and estimation/inference---is more than bookkeeping. It assigns the validity argument to a human-owned, contestable gate. The construct definition, candidate menu, restrictions, thresholds, and holdout design are commitments that the engine can audit but cannot author.

This gate is valuable precisely because it can fail. An empty $\M^*$, a wide $\Bset^*$, or a failed holdout verdict revises the research program rather than silently selecting a favorable proxy. The fuller implications for benchmark design and for forward-looking automated-social-science systems are developed in Appendices~\ref{app:bench} and~\ref{app:automation}; neither is an empirical result claimed here.
...
For the EconLLM research program, Synthetic Cultural Agents are measurement-generation technologies for culturally patterned preference profiles.
...
$\Bset^*=\{\beta(m):m\in\M^*\}$ and
$[L,U]=[\min\Bset^*,\max\Bset^*]$ when $\M^*\neq\emptyset$.
\end{definition}
...
$|\min_{m\in\M^*}\hat\beta(m)-\min_{m\in\M^*}\beta(m)|\le\max_m|\hat\beta(m)-\beta(m)|\to_p 0$
...
\section{Benchmark design as a measurement-menu application}
\label{app:bench}
% =============================================================================
This appendix records a secondary application rather than a result of the present paper. A benchmark score maps model outputs into a quantity interpreted as ability, safety, knowledge, or another capability. It is therefore a measurement function, and benchmark families can be treated as a finite menu rather than as interchangeable leaderboards.

The relevant menu might contain reasoning suites, safety evaluations, cultural-knowledge tasks, or alternative task formats intended to measure one declared capability. A researcher would define the intended use, document which task families enter the menu, and declare convergent and discriminant evidence before comparing models. Held-out task families can then test whether a selection rule based on observed benchmark behavior predicts performance on evidence not used to select a measure. The point is not that a model must have seen no related training data---that claim is usually unobservable---but that benchmark developers should expose a prospective, difficult-to-game evaluation surface.

The motivation is empirical rather than rhetorical. Construct-validity work on LLM benchmarks emphasizes that benchmark scores can be weakly connected to the capability labels attached to them~\cite{bean2025}. Public benchmark content can also become an optimization target; private content may reduce direct gaming but can weaken public scrutiny, so benchmark governance must trade off contamination resistance and transparency~\cite{truong2026}. The Stanford AI Index reports broad capability-benchmark reporting by leading developers but uneven responsible-AI benchmark reporting~\cite{hai2026}. These observations motivate a measurement-menu protocol; they do not establish that any particular benchmark is invalid.

An implementable future design would fix (i) a capability definition and intended use, (ii) a menu of benchmark families, (iii) cross-method auxiliary criteria, (iv) a held-out task-family split, and (v) a downstream decision functional. The resulting output would be an admissible benchmark set and a range of model comparisons robust to that set. This is a labor-market-relevant extension of the framework, but it remains an appendix because no benchmark study has been run here.

\section{Forward-looking: validation gates in automated social science}
\label{app:automation}
% =============================================================================
The prospect of automated social science is scientifically attractive. Language models can help generate hypotheses, propose coding schemes, construct candidate measures, and simulate respondents~\cite{manning2024}. The relevant design question is not whether any of these steps can be automated, but which commitments must remain inspectable when the workflow becomes agentic.

The framework supplies a deliberately limited answer. Automation may expand the menu or implement a frozen measurement procedure. It should not silently choose the construct definition, rewrite the validity network after observing results, or treat its own outputs as independent evidence. The researcher-owned commitments---the intended interpretation and use, menu membership, restrictions, thresholds, auxiliary-measure provenance, and holdout design---are the social-scientific analogue of an evaluation contract.

This stance is optimistic but not utopian. Lin et al. show that interventions in LLM-simulated-user experiments can induce user drift, and propose negative-control outcomes as a diagnostic~\cite{illusion2026}. Hullman et al. distinguish prompt-engineering or fine-tuning repairs from statistical calibration using auxiliary human data under explicit assumptions~\cite{hullman2026}. In two shadow evaluations on unpublished research questions, Kirgis et al. find that frontier agents completed engineering work but did not substantially answer the research questions~\cite{kirgis2026}. These are reasons to build falsifiable validation records, not reasons to abandon productive human--AI research systems.

The medium-run opportunity is a community registry of construct definitions, frozen score matrices, researcher-authored networks, and reproducible audits. Such a registry would not automate theory or settle construct disputes. It would make disagreements legible: competing networks and menus could be compared openly, empty survivor sets would be preserved rather than hidden, and future measurement generators would confront public validity evidence. That is a more credible aspiration than a closed ``verifiable reward'' for autonomous science.
...
\bibitem{dellrambachan2026} Dell, M., and A.~Rambachan (2026). ``Estimation and Inference with AI-Generated Data.'' NBER Summer Institute Methods Lectures, July 30, 2026. Relevant lecture decks: Dell, ``Construct Validity: Credibility When Measurement Is Cheap'' and ``Measurement in the Age of AI: Discovery, Definition, and Observation''; Rambachan, ``Measurement Without Random Validation Samples: Multiple Measurements and Data Fusion.''
...
\bibitem{halterman2024} Halterman, A., and K.~A. Keith (2026). ``Codebook LLMs: Evaluating LLMs as Measurement Tools for Political Science Concepts.'' \textit{Political Analysis} 34(2): 188--204. \url{https://doi.org/10.1017/pan.2025.10017}.
...
\bibitem{messing2026} Messing, S. (2026). ``Hidden Measurement Error in LLM Pipelines Distorts Annotation, Evaluation, and Benchmarking.'' arXiv:2604.11581v6.
