...
Baseline selection rules and pass-rate comparisons live in the evaluation harness; the engine exposes stage tags, units-split composition, slacks, and holdout verdicts.
...
\cvp{}~2.5.0 recomputes $\M^*$ inside a units bootstrap, reports an empty-replicate rate, labels an additive uncertainty band, attributes boundary measures via $|\mathrm{margin}_m|\le\kappa\cdot\mathrm{SE}_m$, and exposes both restriction-stage and units-split holdout.
...
\cvp{}~2.5.0 already exposes stage-tagged restrictions, units-split composition, and holdout verdicts under synthetic gates.
...
What remains before empirical cells are licensed is (i)~frozen, preregistered inputs for a multi-instrument trust profile, (ii)~an evaluation harness that implements the baselines of Eq.~\eqref{eq:falsifiable}, and (iii)~execution of the study on real survey and AI measures.
This section preregisters that study.
...
\section{Automated research workflows (discussion)}
\label{app:workflows}
% =============================================================================
Figure~\ref{fig:systems} is an epistemic map, not a product architecture.
A natural longer-run question is whether the same separation of layers can support \emph{semi-automated} research loops: an agent proposes menu variants or restriction drafts; a human freezes the construct, network, thresholds, and holdout design; the engine returns audit artifacts ($\M^*$, $\Theta^*$, uncertainty band, holdout verdict, freeze/run-id); and the researcher revises theory or design in light of empty sets, wide ranges, or failed held-out tests.
We discuss that possibility only here.
Main-text claims do not depend on autonomous agents writing validity arguments or selecting scientific thresholds.
Until the package is operational on real frozen inputs (Section~\ref{sec:plan}), automated workflows remain a contingent discussion topic, not a contribution of this paper.
